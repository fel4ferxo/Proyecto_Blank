    ...
    var jexcel = (function(el, options) {
        ...
        var defaults = {
            ...
            text: {
                ...
                addComments: 'A�adir comentarios',
        ...
        obj.setData = function(data) {
            ...
            for (j = 0; j < max_j; j++) {
                for (i = 0; i < max_i; i++) {
                    ...
                    // verifica si esta en la posicion correcta
        ...
        obj.parseValue = function(i, j, value) {
            ...
            if (obj.options.columns[i].mask) {
                ...
                let mvalue = '' + jSuites.mask.run(value, obj.options.columns[i].mask, decimal);
                value.value = mvalue; // por 2
            }
        ...
        obj.createCell = function(i, j, value) {
            ...
            if (obj.options.columns[i].editor) {
                ...
                if (obj.options.columns[i].type == 'hidden') {
                    ...
                    element.name = 'c' + i;
                    element.checked = (value.value == 1 || value.value == true || value.value == 'true') ? true : false;
                    element.onclick = function () {
                        let val = ClaseGlobalJX.getObjetEmpty(); //corregir
                        val.value = this.checked; //corregir
        ...
        obj.createToolbar = function(toolbar) {
            ...
            for (var i = 0; i < toolbar.length; i++) {
                if (toolbar[i].type == 'i') {
                    ...
                    if (toolbar[i].t == '1') toolbarItem.innerHTML = '<span class="fa fa-align-left icon-ctm"></span>';
                    else if (toolbar[i].t == '2') toolbarItem.innerHTML = '<span class="fa fa-align-center icon-ctm"></span>';
                    else if (toolbar[i].t == '3') toolbarItem.innerHTML = '<span class="fa fa-align-right icon-ctm"></span>';
                    else toolbarItem.innerHTML = '<span class="fa fa-bold icon-ctm"></span>';
                    ...
                    if (toolbar[i].t == '5') toolbarItem.innerHTML = '<span class="fa fa-tint icon-ctm"></span>';
                    else toolbarItem.innerHTML = '<span class="fa fa-paint-brush icon-ctm"></span>';
        ...
        obj.openFilter = function(columnId) {
            if (!obj.options.filters) {
                ...
                var options = [];
                for (var j = 0; j < obj.options.data.length; j++) {
                    var k = obj.options.data[j][columnId].value;
                ...
                var optionsFiltered = [];
                optionsFiltered.push({ id: '', name: '(Vac&#237;as)' });
            ...
            if (obj.records[y][x].classList.contains('readonly') == true && !force) {
                ...
                if (obj.options.columns[x].editor) {
                    ...
                    if (obj.options.columns[x].type == 'checkbox' || obj.options.columns[x].type == 'radio') {
                        ...
                        // Update data and cell
                        //por revisar checkbox el correcto funcionamiento  //corregir
                        obj.records[y][x].children[0].checked = (value.value == 1 || value.value == true || value.value == 'true' || value.value == 'TRUE') ? true : false;
                        value.value = obj.records[y][x].children[0].checked?true:false ;
        ...
        obj.updateTableReferences = function() {
            ...
            var updatePosition = function(x, y, i, j) {
                if (x != i) {
                    obj.options.data[j][i].col = i;
                    obj.options.data[j][i].movePosition = true;
                    obj.records[j][i].setAttribute('data-x', i);
                }
                if (y != j) {
                    obj.options.data[j][i].row = j;
                    obj.options.data[j][i].movePosition = true;
                ...
                while (row = data[j]) {
                    ...
                    while (row[i] != null) {
                        ...
                        if (style && style[styleIndex]) {
                            ...
                            objCell.style = style[styleIndex];
                            styleIndex++
                        }
                        i++;
                        if (row[i] != null) {
        ...
        obj.undo = function() {
            ...
            if (obj.historyIndex >= 0) {
                ...
                if (historyRecord.action == 'setValue' || historyRecord.action == 'setStyle') {
