...
(function() {
    ...
    this.getObjetEmpty = function() {
        let obj = new Object();
        obj.Id = this.getIdEmpty();
        obj.value = "";
        obj.col = "";
        obj.row = "";
        obj.statusNew = true;
        obj.RowVersion = this.getRowVersionEmpty();
        return obj;
    };
    ...
    var jexcel = (function(el, options) {
        ...
        var defaults = {
            ...
            text:{
                ...
                showingPage: 'Mostrando p&#225;gina {0} de {1} entrada',
                show: 'Mostrar ',
                search: 'Buscar',
                entries: ' entradas',
                columnName: 'Nombre de la columna',
                insertANewColumnBefore: 'Insertar una nueva columna antes',
                insertANewColumnAfter: 'Insertar una nueva columna despu&#233;s',
                deleteSelectedColumns: 'Eliminar columnas seleccionadas',
                renameThisColumn: 'Cambiar el nombre de esta columna',
                ...
                thisActionWillDestroyAnyExistingMergedCellsAreYouSure: 'Esta acci&#243;n destruir&#225; las celdas fusionadas existentes. ¿Est&#225;s seguro?',
                thisActionWillClearYourSearchResultsAreYouSure: 'Esta acci&#243;n borrar&#225; los resultados de la b&#250;squeda. ¿Est&#225;s seguro?',
                thereIsAConflictWithAnotherMergedCell: 'Hay un conflicto con otra celda fusionada',
                invalidMergeProperties: 'Propiedades de fusi&#243;n no v&#225;lidas',
                cellAlreadyMerged: 'Celda ya fusionada',
                noCellsSelected: 'No se seleccionaron celdas',
            },
            // About message
            about: true,
        };
        ...
        obj.setData = function(data) {
            ...
            let Desplazados = []; // objetos que se reemplazaran en el ajuste de posicion
            for (j = 0; j < max_j; j++) {
                for (i = 0; i < max_i; i++) {
                    if (obj.options.data[j] == undefined) {
                        obj.options.data[j] = [];
                    } 
                    // si es nulo crea un objeto vacio
                    if (obj.options.data[j][i] == undefined) {
                        let newObjectEmpty = ClaseGlobalJX.getObjetEmpty();
                        newObjectEmpty.row = j;
                        newObjectEmpty.col = i;
                        obj.options.data[j][i] = newObjectEmpty;
                    }
                    // verifica si esta en la posicion correcta (falta mejorar para columnas)
                    /*const objeto = obj.options.data[j][i];
                    if (objeto && (objeto.row !== j || objeto.col !== i)) {
                        let objIncorrectPosition = { ...obj.options.data[objeto.row][objeto.col] }

                        let newObjectEmpty = ClaseGlobalJX.getObjetEmpty();
                        newObjectEmpty.row = j;
                        newObjectEmpty.col = i;
                        obj.options.data[j][i] = newObjectEmpty;
                        obj.options.data[objeto.row][objeto.col] = objeto;

                        // solucion 1
                        if (Object.keys(objIncorrectPosition).length > 0)
                            Desplazados.push(objIncorrectPosition)
                        // solucion 2
                        *//*while (Object.keys(objIncorrectPosition).length > 0) {
                            const ob = { ...objIncorrectPosition };
                            objIncorrectPosition = { ...obj.options.data[j][objIncorrectPosition.col] }
                            obj.options.data[ob.row][ob.col] = ob
                        }*//*
                    }*/
                }
            }
            ...
            // solucion 1
            for (let i = 0; i < Desplazados.length; i++) {
                obj.options.data[Desplazados[i].row][Desplazados[i].col] = Desplazados[i]
            }

            //rellenar objetos vacios si la posicion no corresponde ejemplo
            /*let myArray = [[{ val: "a", row: 0, col: 0 }, { val: "b", row: 0, col: 1 }],
            [{ val: "c", row: 1, col: 0 }, { val: "d", row: 1, col: 3 }],
                [{ val: "e", row: 2, col: 0 }, { val: "g", row: 2, col: 3 }]]

            for (let row = 0; row < myArray.length; row++) {
                for (let col = 0; col < myArray[row].length; col++) {

                    if (!myArray[row][col]) {
                        myArray[row][col] = { val: "", row, col };
                    }
                    const objeto = myArray[row][col];
                    if (objeto && (objeto.row !== row || objeto.col !== col)) {
                        myArray[row][col] = { val: "", row, col };
                        myArray[objeto.row][objeto.col] = objeto;
            ...
            if (obj.options.lazyLoading == true) {
                ...
                var startNumber = 0
                var finalNumber = obj.options.data.length < 50 ? obj.options.data.length : 50;
        ...
        */
        ...
        obj.parseValue = function(i, j, value, cell) {
            if ((''+value.value).substr(0,1) == '=' && obj.options.parseFormulas == true) {
                let mvalue = obj.executeFormula(value.value, i, j);
                value.value = mvalue; //por analizar, en este instante el se ejecuta la formula y se pierde el formula por reemplazar directamente el value.value
            }
            ...
            // si se quiere dejar de mostrar object y mostrar directamente el valor retorna directamente value.value
            return value.value;
        }
        ...
        obj.createCell = function(i, j, value) {
            ...
            if (obj.options.columns[i].editor) {
                ...
                if (obj.options.columns[i].type == 'hidden') {
                    ...
                    element.classList.add("ck-select");
                    element.type = obj.options.columns[i].type;
                    element.name = 'c' + i;
                    let ck = (value.value == 1 || value.value == true || value.value == 'true') ? true : false;
                    element.checked = ck;
                    //objeto vacio para el valor de chackbox
                    let val = null;
                    if (value) {
                        val = value;
                    } else {
                        val = ClaseGlobalJX.getObjetEmpty();
                        val.row = j;
                        val.col = i;
                        val.value = ck;                       
                    }
                    ...
                    element.onclick = function () {
                        ...
                        //value.value = this.checked;//corregir
                        obj.setValue(td, val);
                    }
        ...
        obj.createCellHeader = function(colNumber) {
            ...
            if (obj.options.stripHTML) {
                if (obj.options.columns[colNumber].direction === "vertical") {
                    obj.headers[colNumber].className = 'td-vertical';
                    obj.headers[colNumber].innerHTML = '<div><span>' + obj.options.columns[colNumber].title + '</span></div>';
                } else {
            ...
            if (cell.classList.contains('readonly') == true) {
                ...
                if (obj.options.columns[x].editor) {
                    ...
                    if (obj.options.columns[x].type == 'hidden') {
                        ...
                        // Value
                        var value = empty == true ? ClaseGlobalJX.getObjetEmpty() : obj.options.data[y][x];
        ...
        obj.closeEditor = function(cell, save) {
            ...
            if (save == true) {
                ...
                if (obj.options.data[y][x].value == value) {
                    cell.innerHTML = obj.edition[1];
                } else {
                    // agregamos estado de edicion
                    let cloneObj = { ...obj.options.data[y][x] }
                    cloneObj.value = value;
                    cloneObj.statusEdit = true;
                    cloneObj.oldValue = { ...obj.options.data[y][x] }
                    obj.setValue(cell, cloneObj);
                }
            } else {
                if (obj.options.columns[x].editor) {
                    // Custom editor
                    ...
                    if (keys.length > 0) {
                        for (var i = 0; i < keys.length; i++) {
                            if (typeof(cell[i]) == 'string') {
                             ...
                             // Update cell
                            if (x != null && y != null) {
                                //por si viene vacio y string caso ctrl+x
                                if (typeof (value) == 'string' && value == "") {
                                    value = { ...obj.options.data[y][x] };
                                    value.value = "";
                                    value.col = x;
                                    value.row = y;
                                    value.statusEdit = true;
                                }
                                records.push(obj.updateCell(x, y, value, force));
                                obj.updateTable(x, y);
                                // Update all formulas in the chain
                ...
                if (obj.options.columns[x].editor) {
                    ...
                    if (obj.options.columns[x].type == 'checkbox' || obj.options.columns[x].type == 'radio') {
                        ...
                        if (typeof value == "object") {
                            obj.records[y][x].children[0].checked = (value.value == 1 || value.value == true || value.value == 'true' || value.value == 'TRUE') ? true : false;
                            value.value = obj.records[y][x].children[0].checked ? false : true;
                            //obj.options.data[y][x] = obj.records[y][x].children[0].checked;
                            obj.options.data[y][x] = value;
                            obj.records[y][x].children[0].checked = value.value;
                        }
        ...
        obj.setStyle = function(o, k, v, force, ignoreHistoryAndEvents) {
            var newValue = {};
            var oldValue = {};
            var records = []; // nuevo agregar para guardar historial del objeto
                ...
                if (typeof(o) == 'string') {
                    ...
                    for (var i = 0; i < o.length; i++) {
                        ...
                        if (! oneApplication[cellName]) {
                            ...
                            let oldval = { ...obj.options.data[y][x] };
                            obj.options.data[y][x].style = o[i].getAttribute('style');
                            obj.options.data[y][x].statusEdit = true;
                            var record = {
                                x: x,
                                y: y,
                                col: x,
                                row: y,
                                newValue: obj.options.data[y][x],
                                oldValue: oldval,
                            }
                            records.push(record);
            ...
            if (obj.options.allowInsertRow == true) {
                ...
                for (var row = rowIndex; row < (numOfRows + rowIndex); row++) {
                    ...
                    for (var col = 0; col < obj.options.columns.length; col++) {
                        // por agregar funcionalidad de verificar si el objeto esta en la posicion
                        if (data[col]) {
                            obj.options.data[row][col] = data[col];
                        } else {
                            let newObjectEmpty = ClaseGlobalJX.getObjetEmpty();
                            newObjectEmpty.col = col;
                            newObjectEmpty.row = row;
                            obj.options.data[row][col] = newObjectEmpty;
                        }
                ...
                for (var row = 0; row < obj.options.data.length; row++) {
                    ...
                    for (var col = columnIndex; col < (numOfColumns + columnIndex); col++) {
                        ...
                        if (data[row])
                            ...
                            let obj = ClaseGlobalJX.getObjetEmpty();
                            obj.col = col;
                            obj.row = row;
                            value = obj;
                        }
                        //var value = data[row] ? data[row] : ClaseGlobalJX.getObjetEmpty();
         ...
         */
        obj.updateTable = function(xx, yy) {
            // Check for spare
            ...
            if (typeof(obj.options.updateTable) == 'function') {
                ...
                //Si existe coordenadas actualizar solo esa celda
                if (xx && yy) {
                    obj.options.updateTable(el, obj.records[yy][xx], xx, yy, obj.options.data[yy][xx], obj.records[yy][xx].innerText, jexcel.getColumnNameFromId([yy, xx]));
                } else {
                    for (var j = 0; j < obj.rows.length; j++) {
                        for (var i = 0; i < obj.headers.length; i++) {
                ...
                if (item > 0) {
                    for (var j = 0; j < 30; j++) {
                        ...
                        if (item > -1) {
                            ...
                            if (obj.tbody.children.length > 50) {
        ...
        obj.loadDown = function() {
            ...
            //para objetos nuevos reajuste la cantidad de elementos visibles
            var itemFirstChild = parseInt(obj.tbody.firstChild.getAttribute('data-y'));
            var itemLastChild = parseInt(obj.tbody.lastChild.getAttribute('data-y'));
            if (itemLastChild - itemFirstChild > 50) {
                for (var j = itemLastChild; j >= itemFirstChild+50; j--) {
                    obj.tbody.removeChild(obj.tbody.lastChild);
                }
            }
            var test = 0;
        ...
        obj.loadValidation = function() {
            if (obj.selectedCell) {
                var currentPage = parseInt(obj.tbody.firstChild.getAttribute('data-y')) / 50;
                var selectedPage = parseInt(obj.selectedCell[3] / 50);
                var totalPages = parseInt(obj.rows.length / 50);
            ...
            for (var j = 0; j < y; j++) {
                ...
                for (var i = 0; i < x; i++) {
                    ...
                    if (! highlighted || obj.records[j][i].classList.contains('highlight')) {
                        ...
                        // Values
                        var value = obj.options.data[j][i].value;
        ...
        obj.paste = function(x, y, data) {
            ...
            if (x != null && y != null && data) {
                ...
                while (row = data[j]) {
                    ...
                    while (row[i] != null) {
                        //recupara el object
                        let objCell = { ...obj.options.data[rowIndex][colIndex] };
                        objCell.value = row[i]
                        objCell.statusEdit = true;
                        // Update and keep history
                        var record = obj.updateCell(colIndex, rowIndex, objCell);
                        // Keep history
                        records.push(record);
                        // Update all formulas in the chain
        ...
        obj.undo = function() {
            ...
            if (obj.historyIndex >= 0) {
                ...
                } else if (historyRecord.action == 'setValue' || historyRecord.action == 'setStyle') {
            ...
            if (obj.options.lazyLoading == true) {
                if (jexcel.timeControlLoading == null) {
                    jexcel.timeControlLoading = setTimeout(function () {
                        ...
                        } else if (obj.content.scrollTop <= 10) {
                            if (obj.loadUp()) {
